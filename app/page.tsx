...
export default function Home() {
  ...
  // Lenis smooth scroll — client-side only, no SSR
  useEffect(() => {
    ...
    const initLenis = async () => {
      try {
        ...
        lenis = new Lenis({ duration: 1.2, touchMultiplier: 2 })
  ...
  return (
    <>
      {/* Soft-glow cursor — desktop only, disabled on touch */}
      <CustomCursor />

      {/* Skip-to-content — keyboard users */}
      <a
        href="#main"
        className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-[100] focus:px-4 focus:py-2 focus:bg-[var(--accent-blue)] focus:text-white focus:rounded-[8px] focus:font-semibold focus:text-sm"
      >
      ...
      >
        <LeadForm defaultRole={modalRole} onSuccess={() => undefined} />
      </Modal>
    </>
  )
}
