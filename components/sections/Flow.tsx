...
function DesktopFlow() {
  ...
  useEffect(() => {
    return scrollYProgress.on('change', (v) => {
      ...
      setProgress(c)
      setActiveStep(Math.min(STEPS.length - 1, Math.floor(c * STEPS.length)))
    })
  }, [scrollYProgress])
...
export default function Flow() {
  ...
  return (
    ...
    >
      ...
      {/* Mobile stepper */}
      <div className={`section-padding ${reduced ? 'block' : 'lg:hidden'}`}>
        <MobileFlow />
      </div>
